#ifndef __CANSERVOCTRL_H__
#define __CANSERVOCTRL_H__

#include "servoctrl_if.hpp"
#include "servocanin.hpp"
#include "servocanout.hpp"

typedef std::map<uint16_t, CanInServoItemPtr> CanInServoItemPtrMap;

class CanServoCtrl: public ServoCtrlIf
{
public:
    CanServoCtrl();
    virtual ~CanServoCtrl();
    ...
    ServoCtrlErrCode WaitForHome(const ServoInfoPtrVec &servo_array, int timeout_ms = -1);
    ...
    ServoCtrlErrCode WaitForReady(const ServoInfoPtrVec &servo_array, int timeout_ms = -1);
     ...
     * @return    		SERVO_ERR_OK on success, otherwise failed.
     */
    ServoCtrlErrCode ReadState(CanInServoItemPtrMap &servo_array);

    /**
     * @brief       Enable/disable servos silent mode(by ServoInfoPtrMap)
     * @param[in]	servo_map, set servos silent mode according to the 'id' field of input servos map.
     * @param[in]   enable, enable or disable silent mode
     * @param[in]   timeout_ms, wait at most timeout_ms for operation compete, -1 for wait forever
     * @return      SERVO_ERR_OK on success, otherwise failed.
     */
    ServoCtrlErrCode WaitForSilent(const ServoInfoPtrMap &servo_map, bool enable, int timeout_ms);

private:
    static ServoRawInfoMap head_map;
    static ServoRawInfoMap leftarm_map;
    static ServoRawInfoMap leftfinger_map;
    static ServoRawInfoMap rightarm_map;
    static ServoRawInfoMap rightfinger_map;
    static ServoRawInfoMap total_map;

    ServoCanIn *head_canin;
    ServoCanIn *leftarm_canin;
    ServoCanIn *leftfinger_canin;
    ServoCanIn *rightarm_canin;
    ServoCanIn *rightfinger_canin;

    ServoCanOut *head_canout;
    ServoCanOut *leftarm_canout;
    ServoCanOut *leftfinger_canout;
    ServoCanOut *rightarm_canout;
    ServoCanOut *rightfinger_canout;
};
